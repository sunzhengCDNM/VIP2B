...
def norm_method(feature_data, nm):
	# RobustScaler
	if nm == 'RobustScaler':
		from sklearn.preprocessing import RobustScaler
		norm_df = pd.DataFrame(RobustScaler().fit_transform(feature_data))

	# MinMaxScaler
	elif nm == 'MinMaxScaler':
		from sklearn.preprocessing import MinMaxScaler
		norm_df = pd.DataFrame(MinMaxScaler().fit_transform(feature_data))

	# Normalizer
	elif nm == 'Normalizer':
		from sklearn.preprocessing import Normalizer
		norm_df = pd.DataFrame(Normalizer().fit_transform(feature_data))

	# Z-Score
	elif nm == 'Z-Score':
		from sklearn.preprocessing import StandardScaler
		norm_df = pd.DataFrame(StandardScaler().fit_transform(feature_data))

	# None
	else:
		norm_df = feature_data

	norm_df.columns = feature_data.columns
	return norm_df


def pre_tra_data(df, nm):
	sub_df = df.copy()
	...
	return norm_method(sub_df, nm)

def custom_predict(proba, threshold):
...
def main():
	...
	parser.add_argument('-T',help='the threshold with predict_proba for predict, default 0.5',dest='p_threshold',type=float,default=0.5)
#	parser.add_argument('-s',help='species that need to be screened, use commas to join index, default human',dest='screened',type=str,default='human')
	parser.add_argument('-s',help='species that need to be screened, use commas to join index, default none',dest='screened',type=str,default='')
	parser.add_argument('-n',help='homogenization method, choice from RobustScaler/MinMaxScaler/Normalizer/Z-Score/none',dest='nm',choices=['RobustScaler', 'MinMaxScaler', 'Normalizer', 'Z-Score', 'none'],default='none')
	...
	for file in file_list:
		...
		filter_s = 'Taxonomy != "' + '" & Taxonomy != "'.join(args.screened.split(',')) + '"'
		df = df.query(filter_s)
		df = df[(df['G_Score'] > args.threshold)].reset_index(drop=True)
		sub_df = df.iloc[:,[0, 2, 4, 1, 8]] # Taxonomy  Sequenced_Tag_Num  Sequenced_Reads_Num  Theoretical_Tag_Num  G_Score
		tra_df = pre_tra_data(sub_df, args.nm)
		if args.type == 1:
#			pred_list = clf.predict(tra_df[feature_list])
			prob_list = clf.predict_proba(tra_df[feature_list])[:,1]
			pred_list = custom_predict(prob_list, args.p_threshold) 
			...
			c_sub_df_tmp['Taxonomy'] = merge_spe_list
			c_sub_df = c_sub_df_tmp.groupby('Taxonomy').agg({'Sequenced_Tag_Num':'sum', 'Sequenced_Reads_Num':'sum', 'Theoretical_Tag_Num':'sum'}).reset_index(drop=False)
			c_sub_df['G_Score'] = np.sqrt(c_sub_df['Sequenced_Tag_Num'] * c_sub_df['Sequenced_Reads_Num'])
			c_tra_df = pre_tra_data(c_sub_df, args.nm)
			spe_list = list(c_tra_df['Taxonomy'])
#			pred_list = clf.predict(c_tra_df[feature_list])
		...
		if 'tag' in df:
			if args.type == 2:
				tag_sub_df = c_sub_df_tmp.groupby('Taxonomy').agg({'tag':'sum'}).reset_index(drop=False)
		...
		tra_df.rename(columns={'Taxonomy': 'Taxonomy'}, inplace=True)
		if args.output:
